...
function Connection(config) {
  ...
  if (config.socksProxy) {
    this._config.socksProxy = {
      host: config.socksProxy.host,
      port: config.socksProxy.port,
      type: config.socksProxy.type
    }

    if (config.socksProxy.authentication) {
      this._config.socksProxy.authentication = {
        username: config.socksProxy.authentication.username,
        password: config.socksProxy.authentication.password
      };
    }
  }
...
Connection.prototype.connect = function() {
  ...
  if (config.socksProxy) {
    var socksOptions = {
      proxy: {
          ipaddress: config.socksProxy.host,
          port: config.socksProxy.port,
          type: config.socksProxy.type
      },
      target: {
    ...
    if (config.socksProxy.authentication) {
      socksOptions.proxy.authentication = {
        username: config.socksProxy.authentication.username,
        password: config.socksProxy.authentication.password
      };
    }

    Socks.createConnection(socksOptions, function(err, socket) {
      if (err) {
        self.emit('error', err);
        return;
      }
    ...
    socket.once('connect', function onConnectSuccess() {
      self.removeListener('error', onConnectError);
      self._connect(socket);
    }.bind(self));
  }
};

Connection.prototype._connect = function(socket) {
  var self = this;
